...
### Критические исправления стабильности (потеря тренировки на мобильном)
- **Двойная запись в Firestore**: убран второй `setActiveWorkout()` в `saveCurrentWorkout()` — он перезаписывал документ без upражнений, что потенциально Could приводить к потере данных при гонке запросов
- **Гонка состояния auth**: при перезагрузке вкладки Firebase Auth ещё не инициализирован, из-за чего `userId` не совпадал и тренировка не восстанавливалась — добавлен `updateUserId()` перед запросом к Firestore
- **Атомарная очистка**: убрана небезопасная пара `deleteDoc + setDoc` при завершении тренировки — заменено на один `setDoc` чтобы избежать потери данных при обрыве сети между двумя операциями

## [2.1.0] - 2026-03-30

### Исправлено
- Логика группировки весов: теперь подходы группируются только если они идут подряд с одним весом. Это сохраняет хронологический порядок тренировки (пирамиды и т.д.)
- Баг инициализации Firebase: исправлен порядок создания хранилища (Storage) для предотвращения ошибок при загрузке приложения
- Синхронизация отображения истории с новой логикой группировки
